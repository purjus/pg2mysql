...
function pg2mysql($input, $header = true)
{
    ...
    while (isset($lines[$linenumber])) {
        ...
        if ($in_create_table) {
            ...
            $line = str_replace(" timestamp DEFAULT now()", " datetime DEFAULT NOW()", $line);
            $line = preg_replace("/ timestamp( NOT NULL)?(,|$)/", ' datetime ${1}${2}', $line);
        ...
        if (1 === preg_match('/DROP DATABASE \"?([0-9A-Za-z_]+)\"?/', $line, $matches)) {
